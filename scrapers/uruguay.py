...
def fetch_standings():
    """Standings de la Primera División de Uruguay desde ESPN."""
    try:
        r = requests.get(
            'https://site.api.espn.com/apis/v2/sports/soccer/uru.1/standings',
            headers=HEADERS, timeout=10
        )
        r.raise_for_status()
        data = r.json()

        # Los standings están en children (puede haber Apertura/Clausura/Anual)
        all_entries = []
...
def fetch_goals():
    ...
    for league_name in ['uruguay', 'urupd', 'uruprimera']:
        try:
            ...
            continue
    logger.warning("Goals no disponibles en soccerstats para Uruguay — usando defaults")
    return {}


def main():
    logger.info("🔥 Liga: Primera División Uruguay (via ESPN)")
    ...
    with open('static/uruguay_stats.json', 'w', encoding='utf-8') as f:
        json.dump(combined_data, f, indent=2, ensure_ascii=False)
    print(f"\n✅ Archivo generado: static/uruguay_stats.json")

...
# Aliases vistos en torneos CONMEBOL (variaciones comunes)
# OJO: "Nacional" en Paraguay choca con "Nacional" de Uruguay; el motor
# debe resolver vía contexto de partido. Por eso solo agregamos
# "Nacional Asunción" como alias explícito.
TEAM_ALIASES = {
    "Olimpia":            "Club Olimpia",
    "Olimpia Asunción":   "Club Olimpia",
    "Guaraní":            "Guarani",
...
def fetch_standings():
    """Standings de la Primera División de Paraguay desde ESPN."""
    try:
        r = requests.get(
            'https://site.api.espn.com/apis/v2/sports/soccer/par.1/standings',
...
def fetch_goals():
    ...
    for league_name in ['paraguay', 'parprimera', 'paragdivA']:
        try:
            ...
            continue
    logger.warning("Goals no disponibles en soccerstats para Paraguay — usando defaults")
    return {}


def main():
    logger.info("🔥 Liga: Primera División Paraguay (via ESPN)")
    ...
    # Aliases CONMEBOL: nombres alternativos apuntan al mismo dict canónico.
    # Si el alias ya existe (soccerstats lo devolvió), mergeamos campos no
    # vacíos antes de sobrescribir.
    ...
    with open('static/paraguay_stats.json', 'w', encoding='utf-8') as f:
        json.dump(combined_data, f, indent=2, ensure_ascii=False)
    print(f"\n✅ Archivo generado: static/paraguay_stats.json")
